...
duckdb.sql(
    ...
    f'AS SELECT * FROM read_parquet("FD_INDCVI_2020.parquet")'
    )
duckdb.sql(
    'CREATE OR REPLACE VIEW table_logement ' +\
    f'AS SELECT * FROM read_parquet("FD_LOGEMT_2020.parquet")'
    )
duckdb.sql(
    'CREATE OR REPLACE VIEW documentation_indiv ' +\
    f'AS {renommage_documentation} ' +\
    f'FROM read_csv_auto("dictionnaire_variables_indcvi_2020.csv", header=true)'
    )
duckdb.sql(
    'CREATE OR REPLACE VIEW documentation_logement ' +\
    f'AS {renommage_documentation} ' +\
    f'FROM read_csv_auto("dictionnaire_variables_logemt_2020.csv", header=true)'
    )
```
