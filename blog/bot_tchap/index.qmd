---
title: "Bref, j'ai créé un bot sur Tchap banché à un LLM"
...
Il faut aussi, à partir de l'ID d'un événement, il faut : 
...
Pour que la réponse de notre assistant préféré passe bien dans Tchap, il faut lui ajouter un prompt 
système qui demande de répondre en markdown sans échapper les caractères spéciaux. 

Après, il faut gérer les problèmes de fonction `async` dans Python. 
Je n'ai pas tout compris mais ce que j'ai compris c'est que les demandes "compliquées", 
c'est à dire dont la réponse peut prendre un peu de temps, on met un `await` devant. 
...
Pour la simplicité de ce post, j'ai simplifié le contenu du repo.
Il n'y a pas d'organisation en package Python ici, tout est *peu proprement* placé dans un seul script `main.py`.
Allez voir le repo pour une cible plus propre. 
...
de ma machine. L'équipe du SSP Cloud m'a bien guidé (et mon assistant IA préféré aussi).
...
Bon après, le `Dockerfile` est assez basique et un assitant IA m'a bien aidé à le rédiger. 
...
LLM) ?*. Bon, après vérification, ils ne sont pas poussées dans l'image Docker. 
