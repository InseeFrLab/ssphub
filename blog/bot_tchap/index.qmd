...
Si vous n'êtes pas un freak, vous pouvez remplacer les deux variables `${SSP_USERNAME}` et
`${DOCKERHUB_USERNAME}` dans le contrat yaml ci-dessus. 
Vous pouvez alors lancer le déployement avec `kubectl apply -f`.
...
::: {.callout-tip title="Tuer le pod"}

Comme on a spécifié qu'on **voulait toujours un pod actif dans notre contrat**, un simple `kubectl delete pod <pod-name> -n <namespace>` va le tuer mais le 
...
- soit on arrête le déploiement avec `kubectl delete deployment <name> -n <namespace>`
- soit on lui de deployer 0 pod avec `kubectl scale deployment <name> --replicas=0 -n <namespace>`
