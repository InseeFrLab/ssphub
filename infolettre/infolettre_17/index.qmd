...
Ces réseaux de neurone étant très gourmands en calculs du fait
...
Dans l'image ci-dessous, par exemple, une base de données vectorielle pourra 
...
`Python` qui est trop généraliste pour cela. 
...
Le gain de popularité de `DuckDB` au cours de l'année 2024 
...
Ce langage est depuis des années le langage de la _data visualisation_.
Néanmoins, grâce au 
développement
de librairies `Javascript` {{< fa brands js-square >}} pour la manipulation
de données et de plateformes de partage comme [`Observable`](https://observablehq.com), 
les spécialistes de _data visualisation_ et les _data scientists_
...
une librairie développée par _Posit_ qui
porte la grammaire `R` dans le navigateur (exemple à tester 👇️).
Ce portage de `R` s'inspire de celui qui existait déjà
en `Python` avec [`Pyodide`](https://pyodide.org/en/stable/usage/quickstart.html).
Cette approche est assez similaire à celle adoptée 
dans la rétropsective du réseau en 2023 ([infolettre #16](/infolettre/infolettre_16)),
à l'exception
que cette page privilégiait l'utilisation directe de `Javascript` plutôt que
l'intermédiaire d'une grammaire `R`. 
...
`Javascript` dans des applications `R` ou `Python` par le biais
de `Shiny`, de `Dash` ou de `Streamlit`. 
...
services spécialisés ou la mise en place de serveurs dédiés capables
d'être  XXX
