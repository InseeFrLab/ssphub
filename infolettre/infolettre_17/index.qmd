...
dans des encadrés qui sont déroulables ou sont référencés dans les sections _"Pour en savoir plus"_.  
...
d'énergies et plus coûteuses à l'achat du fait d'une demandant excédant l'offre,
...
qui a été mis en oeuvre pour évaluer la pertinence des réponses proposées par le modèle
...
Sur un plan plus technique, `Python` reste le [point d'entrée de référence](https://www.tiobe.com/tiobe-index/)
...
qui ont été diffusées dans ce format, accompagnées d'un [guide sur ce blog](/post/parquetRP) (plus de détails dans l'[infolettre #16](/infolettre/infolettre_16)). 
...
(comme les formats `.sas7bdat`, `dbase`, etc.). Par exemple, les données détaillées du 
...
au format `Parquet`
rend `DuckDB` particulièrement approprié pour le traitement de données volumineuses (voir la partie plus technique). 
...
Le __29 février entre 15h et 16h__, Eric Mauvière nous fera une présentation, avec de nombreux exemples
issus de la statistique publique, de la manière dont une visualisation de données peut être construite
pour transmettre un message clair aux lecteurs. Cette
présentation aura lieu sur [Zoom](https://insee-fr.zoom.us/j/92487784559?pwd=WVdsNUFNWXhUNnJYUW56WFdyNHlNdz09), elle
est ouverte à tous. Pour les agents de la DG de l'Insee, vous pouvez venir en __4-C-458__ assister à la présentation et
échanger à l'issue de celle-ci. 

Cette présentation permettra d’évoquer les bonnes pratiques, les règles opérationnelles et les outils simples pour construire des visualisations et valorisations de données, des graphiques faciles à lire et à comprendre, permettant de rendre leur message intelligible, efficace et utile à un spectre large de publics.

Éric Mauvière est statisticien, passé par la diffusion et les études régionales de l'Insee. Il a ensuite créé le logiciel cartographique web `Géoclip` utilisé, par exemple, par le site [https://statistiques-locales.insee.fr/](https://statistiques-locales.insee.fr/#c=home). Plus récemment, il a participé à la conception du site [https://vizagreste.agriculture.gouv.fr/](https://vizagreste.agriculture.gouv.fr/), portail de visualisation de données du service statistique ministériel du Ministère de l'Agriculture et de la Souveraineté Alimentaire. Depuis 3 ans, au sein d'`Icem7`, il participe à la diffusion de connaissances sur les problématiques de _dataviz_ et forme sur mesure en sémiologie graphique et analyse de données.

- [Invitation outlook](XXXXX)
...
permet à des experts de la donnée de l'administration de consacrer jusqu'à 10% de leur temps de travail afin de monter 
en compétence en participant à des projets transversaux, à des formations ou encore à des moments informels pour
...
__auto-formation de découverte de `Python`__, construite par
les équipes innovation de l'Insee.
Il s'agit d'une expérimentation d'un nouveau
mode de formation : l'e-formation tutorée. 

En principe, la formation a lieu sur une durée longue (2 mois), période pendant laquelle vous pouvez exécuter à votre rythme les différents notebooks disponibles. Chaque chapitre de formation, disponible sous la forme de _notebooks jupyter_, peut être testé en un clic à partir du [catalogue de formation](https://www.sspcloud.fr/formation?search=&path=%5B%22Initiation%20%C3%A0%20Python%22%5D) du `SSPCloud`.
Ces ressources restent d'ailleurs disponibles en continue pour permettre de revenir dessus ultérieurement si besoin. 
Ce système de formation a l'avantage à la fois de favoriser une pédagogie par la pratique continue qui a fait ses preuves pour l'apprentissage des langages de programmation, tout en laissant à chacun la possibilité d'avancer à son rythme. 

Ces ressources sont accompagnées d'un système de mentorat prenant deux formes :

- tout au long de la formation : vous pouvez poser toutes vos questions sur un canal Tchap dédié, auxquelles les mentors de la formation répondent rapidement, afin de ne jamais rester bloqué ;
- ponctuellement : une visio est réalisée avec 1-2 mentors afin de pouvoir discuter plus en détail des problèmes que vous pouvez rencontrer.

Un [canal Tchap `SSPy - Formation "Initiation à Python"`](https://matrix.to/#/#sspy:agent.finances.tchap.gouv.fr) est disponible pour les agents publics ayant un compte `Tchap` afin d'échanger sur cette formation (poser des questions de compréhension, comprendre un _bug_, assister les collègues bloqués, etc.).
Les premiers formés ont commencé leur apprentissage en ce début d'année 2024. Il est possible de les rejoindre à tout moment si vous désirez découvrir `Python` ! 