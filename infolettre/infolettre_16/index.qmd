...
- animation noel ?
- spoiler
- mise en forme events.json
- vérifier liens vers publis ssphub
- box making of
- show/hide code
------>
...
```{ojs}
//| echo: false
html`<div>Pendant l'année 2022, le réseau a ainsi connu <span class="underline-big">${events.length}</span> événements et publications de contenu (${countEvents("Infolettre")}, ${countEvents("Post de blog")}, ${countEvents("Evénement virtuel ou présentiel")}, ${countEvents("Masterclass")}).</div><br>`
```
...
très différentes: Sam Altman, le PDG, 
était licencié par le conseil d'administration
avant d'être réembauché. 
Parmi les hypothèses pour expliquer cette crise,
l'une des plus récurrentes serait des
désaccords entre le comité d'administration et les équipes
d'OpenAI sur les recherches menées dans le cadre d'un projet
nommé $Q^*$ d'une intelligence artificielle en mesure de résoudre
...
- Une [dépêche de _Reuters_](https://www.reuters.com/technology/sam-altmans-ouster-openai-was-precipitated-by-letter-board-about-ai-breakthrough-2023-11-22/)

:::

## L'Europe adopte un _"AI Act"_ historique

Les Etats membres et le Parlement européen sont parvenus à un accord le 9 décembre
sur des règles régissant les systèmes d’intelligence artificielle dans un accord nommé
"Artificial Intelligence Act". 
Cet accord est un compromis entre une régulation forte
protégeant les données produites quotidiennement
par les citoyens européens
et une approche plus permissive, défendue
...
dernières. Néanmoins,
concernant celles-ci, le compromis prévoit une approche à deux vitesses. 
Outre le respect des règles européennes 
de propriété intellectuelle, les développeurs de modèles génératifs
devront s'assurer que les produits diffusés soient bien identifiés comme
...
L'AI Act interdit également l'utilisation de l'IA dans quelques domaines, jugés trop sensibles. 
Par exemple, la reconnaissance faciale de masse est interdite, hormis lorsque celle-ci
est justifiée par des motifs de sécurité nationale.  
D'autres utilisations, porteuses potentiellement de dérives, comme
...
- Un [article du _Monde_](https://www.lemonde.fr/economie/article/2023/12/09/ai-act-l-union-europeenne-pionniere-dans-la-regulation-de-l-intelligence-artificielle_6204830_3234.html) (réservé aux abonnés) sur le sujet ;
...
qui les intéresse, l'Insee diffuse également 
...
vise à simplifier l'exploitation
de celles-ci. Pour accompagner cette innovation,
en partenariat avec les services de diffusion de l'Insee, 
le [dernier _post_ de blog](https://ssphub.netlify.app/post/parquetrp/) du réseau présente un guide
...
- Un [article](https://www.insee.fr/fr/information/7635842) sur le format `Parquet` dans le _Courrier des stats n°9_
...
une série d'article sur le format `Parquet`;
...
### Le [SSPCloud](https://datalab.sspcloud.fr) se dote d'un visualiseur de fichiers basé sur `DuckDB`
...
clients: {{< fa brands r-project >}}, {{< fa brands python >}}
mais aussi un navigateur web. 
Plusieurs acteurs majeurs de l'écosystème de
la _data science_ (
[HuggingFace](https://huggingface.co/docs/hub/datasets-viewer), [Observable](https://observablehq.com/documentation/data/databases/database-clients#duckdb) notamment) ont
...
de mettre en oeuvre un [explorateur similaire](https://datalab.sspcloud.fr/data-explorer?rowsPerPage=25&page=1&columnWidths=%7B%7D&columnVisibility=%7B%7D) 🚀. 


![Un exemple d'utilisation de cet explorateur sur les données détaillées du recensement 👆️](viewer-sspcloud.png)

Celui-ci s'appuie également sur `DuckDB` et permet de visualiser de 
...
sur le [`SSPCloud`](https://datalab.sspcloud.fr) pour l'utiliser, il suffit d'essayer de lire un
