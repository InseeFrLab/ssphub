...
message = {
  ...
  if (scrubber_participants < end_count - 100) {
    ...
    const message_to_print2 = `En fin d'année, ce sont <span class = "underline-big">${end_count}</span> personnes qui étaient membres de la liste de diffusion 🚀🚀.`
...
```{ojs}
//| echo: false
viewof scrubber_participants = Scrubber(numbers2, {loopDelay: 8000, initial: end_count})
```
