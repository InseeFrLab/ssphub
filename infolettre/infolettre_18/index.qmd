...
Suivent ensuite les actualités du réseau et
...
le réseau de neurone derrière `Sora` a encore des difficultés à respecter certaines
...
que les applications iront bien au-delà de ces secteurs économiques puisque la vidéo occupe une place prédominante dans la diffusion d'information sur internet. 
...
proposées par une IA et sur les capacités de raisonnement de celle-ci. Sur des questions d'un niveau de premier cycle universitaire (métrique MMLU proposée par  [Hendrycks et al. (2020)](https://arxiv.org/abs/2009.03300), Mistral Large propose la bonne réponse dans 81% des cas, ce qui l'amène presque au niveau de GPT-4 (86%) et bien au-dessus de Llama-2 (70%), le 
...
`Observable`. En plus d'être une extension du langage
...
[observablehq.com](https://observablehq.com/) pour simplifier les réutilisations sur d'autres
_notebooks_ du code proposé ou des données sous-jacentes. 
...
publications reproductibles. Ce dernier écosystème permet déjà 
...
Ce virage est similaire à celui pris, depuis quelques années, par `Python`
dans la communauté des _data scientists_
où les _notebooks_, après avoir eu une phase d'hégémonie, sont revenus à leur
fonction initiale: des carnets pour expérimenter avant de passer à des scripts standards 
ou alors de belles pages pour présenter une démarche de manière pédagogique. 
...
##  Chistophe Dervieux, "`Quarto`: Une évolution de `R Markdown` pour des travaux statistiques reproductibles"

Pour fiabiliser la production de documents construits en
valorisant
des données (tableaux, graphiques, etc.), `RStudio` (devenu `Posit` depuis) 
...
devenue incontournable dans l'écosystème `R` et
est dorénavant largement utilisé. Pour étendre
les vertus de cette approche à d'autres langages, 
Posit a commencé à développer `Quarto`, un écosystème
...
Le __23 avril de 15h à 16h__,
Christophe Dervieux (Posit) nous présentera `Quarto`,
l'écosystème de publications reproductibles qui succède à RMarkdown.
Cet événement est proposé de manière hybride: par
le biais de Zoom ou, pour les agents en poste 
...
réseaux. 
Ce questionnaire est utile pour mieux cibler les besoins et les centres d'intérêt pour organiser des événements dans le cadre du réseau.

Afin de connaître les besoins et centre d'intérêt en cette année 2024,
